...
function writePassword() {
    ...
    var intergers =["1", "2", "3", "4", "5", "6", "7", "8","9"]
    var symbols = ["!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "{", "}", "[", "]", "=", "<", ">", "/", ",", "."];
    
    // add arrays to userInput array if value is true
    if(uppers){userInput.push.apply(userInput,uppercase)};
    if(lowers){userInput.push.apply(userInput,lowercase)};
    if(numbers){userInput.push.apply(userInput, intergers)};
    if(special){userInput.push.array(userInput, symbols)};
